...
(source: TopScoreSource[INPUT, E, TObj, GIVEN],
 history: HistorySource[TObj, TRAVERSE_E, TRAVERSED_TO],
 op: TraverseOp[TRAVERSED_TO],
 maxItemsPerTarget: Int)
	extends TopScoreSource[INPUT, TRAVERSE_E, TRAVERSED_TO, GIVEN] {
	
	override def getRecommendations(originId: Id[INPUT], maxCount: Int): TopScores[INPUT, TRAVERSED_TO] = {
		val scores = source.getRecommendations(originId, maxCount)
		
		val topItemScoresByItemId = new mutable.HashMap[Id[TRAVERSED_TO], Score[TRAVERSED_TO]]()
		
		scores.scores.foreach(ts => {
			val targetItems = history.getDestinationIdsFor(ts.similarTargetId, maxItemsPerTarget)
